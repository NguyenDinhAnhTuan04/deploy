...
def sample_stellio_config():
    ...
    return {
        ...
        "logging": {"enable_http_logging": True, "log_bodies": False, "level": "INFO"},
        "performance": {
...
def sample_ngsi_ld_entity():
    ...
    return {
        ...
        "location": {
            "type": "GeoProperty",
            "value": {"type": "Point", "coordinates": [105.123, 21.456]},
        },
    }
...
class TestConfigLoader:
    ...
    def test_load_config_missing_stellio_section(self, tmp_path):
        ...
        with pytest.raises(
            ValueError, match="Configuration file must contain 'stellio' section"
        ):
            loader.load_config()
    ...
    def test_validate_config_missing_endpoints(self, tmp_path):
        ...
        with open(config_file, "w") as f:
            yaml.dump(
                {"stellio": {"base_url": "http://localhost", "api_version": "v1"}}, f
            )
...
class TestBatchPublisher:
    ...
    @responses.activate
    def test_publish_entity_conflict_409(
        self, sample_stellio_config, sample_ngsi_ld_entity
    ):
    ...
    @responses.activate
    def test_publish_entity_retry_on_500(
        self, sample_stellio_config, sample_ngsi_ld_entity
    ):
    ...
    @responses.activate
    def test_publish_entity_max_retries_exceeded(
        self, sample_stellio_config, sample_ngsi_ld_entity
    ):
    ...
    @responses.activate
    def test_publish_entity_non_retryable_error(
        self, sample_stellio_config, sample_ngsi_ld_entity
    ):
    ...
    @responses.activate
    def test_publish_batch_fallback_to_individual(
        self, sample_stellio_config, sample_ngsi_ld_entity
    ):
...
class TestPublishReportGenerator:
    ...
    def test_save_failed_entities(self, sample_stellio_config, tmp_path):
        ...
        results = [
            PublishResult(entity_id="entity1", status_code=201, success=True),
            PublishResult(
                entity_id="entity2", status_code=500, success=False, error="Error"
            ),
            PublishResult(
                entity_id="entity3", status_code=500, success=False, error="Error"
            ),
        ]
...
class TestEntityPublisherAgentIntegration:
    ...
    def test_publish_success(self, sample_config_file, sample_entities_file, tmp_path):
        ...
        report = agent.publish(
            input_file=sample_entities_file, output_report=str(tmp_path / "report.json")
        )
    ...
    @responses.activate
    def test_publish_with_conflicts(
        self, sample_config_file, sample_entities_file, tmp_path
    ):
    ...
    @responses.activate
    def test_publish_with_failures(
        self, sample_config_file, sample_entities_file, tmp_path
    ):
...
class TestPerformance:
    ...
    def test_publish_722_entities_under_30_seconds(
        self, sample_config_file, large_entities_file, tmp_path
    ):
        ...
        report = agent.publish(
            input_file=large_entities_file, output_report=str(tmp_path / "report.json")
        )
...
class TestEdgeCases:
    ...
    def test_all_entities_fail(
        ...
        for _ in range(30):  # 10 entities × 3 retries
            responses.add(
                responses.POST, "http://localhost:8080/ngsi-ld/v1/entities", status=500
            )
    ...
    def test_malformed_entities(self, sample_config_file, tmp_path):
        ...
        with responses.RequestsMock() as rsps:
            ...
            rsps.add(
                responses.POST, "http://localhost:8080/ngsi-ld/v1/entities", status=201
            )
...
if __name__ == "__main__":
    pytest.main(
        [__file__, "-v", "--cov=agents.context_management", "--cov-report=html"]
    )
