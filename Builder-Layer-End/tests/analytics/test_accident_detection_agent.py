...
def test_speed_variance_detector_normal_traffic():
    ...
    detected, confidence, reason = detector.detect(
        observations, "urn:ngsi-ld:Camera:Test"
    )
...
def test_speed_variance_detector_insufficient_data():
    ...
    observations = [
        make_observation("urn:ngsi-ld:Camera:Test", speed=30) for _ in range(3)
    ]
...
def test_occupancy_spike_detector_normal_traffic():
    ...
    observations = [
        make_observation("urn:ngsi-ld:Camera:Test", occ=0.5) for _ in range(25)
    ]
...
def test_occupancy_spike_detector_accident():
    ...
    observations = [
        make_observation("urn:ngsi-ld:Camera:Test", occ=0.3) for _ in range(20)
...
def test_sudden_stop_detector_normal_deceleration():
    ...
    observations = [
        make_observation(
            ...
            speed=50 - i * 3,
            observed_at=f"2025-11-01T10:00:{i:02d}Z",
        )
        for i in range(10)
    ]
...
def test_sudden_stop_detector_accident():
    ...
    observations = [
        make_observation(
            "urn:ngsi-ld:Camera:Test", speed=60, observed_at="2025-11-01T10:00:00Z"
        ),
        make_observation(
            "urn:ngsi-ld:Camera:Test", speed=5, observed_at="2025-11-01T10:00:10Z"
        ),
    ]
...
def test_sudden_stop_detector_low_initial_speed():
    ...
    observations = [
        make_observation(
            "urn:ngsi-ld:Camera:Test", speed=15, observed_at="2025-11-01T10:00:00Z"
        ),
        make_observation(
            "urn:ngsi-ld:Camera:Test", speed=2, observed_at="2025-11-01T10:00:10Z"
        ),
    ]
...
def test_pattern_anomaly_detector_normal_pattern():
    ...
    config = {"name": "pattern_anomaly", "enabled": True, "intensity_threshold": 2.5}
...
def test_pattern_anomaly_detector_accident():
    ...
    observations = [
        make_observation("urn:ngsi-ld:Camera:Test", intensity=10) for _ in range(19)
...
def test_accident_detection_integration(tmp_path, monkeypatch):
    ...
    observations = [
        make_observation(camera_ref, speed=50, occ=0.3) for _ in range(10)
    ] + [
...
def test_false_positive_filtering_min_confidence(tmp_path, monkeypatch):
    ...
    camera_ref = "urn:ngsi-ld:Camera:Test"
    observations = [make_observation(camera_ref, speed=30 + i) for i in range(15)]
...
def test_multiple_methods_detection(tmp_path, monkeypatch):
    ...
    observations = [
        ...
        make_observation(
            ...
            occ=0.9,
            intensity=100,
            observed_at="2025-11-01T10:00:15Z",
        )
        for _ in range(3)
    ]
...
def test_rush_hour_false_positive(tmp_path, monkeypatch):
    ...
    camera_ref = "urn:ngsi-ld:Camera:Test"
    observations = [make_observation(camera_ref, occ=0.3 + i * 0.02) for i in range(30)]
