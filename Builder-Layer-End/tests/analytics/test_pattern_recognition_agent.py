...
def temp_config(tmp_path):
    ...
    config = {
        "pattern_recognition": {
            ...
            "output": {"patterns_file": str(tmp_path / "patterns.json")},
...
def mock_neo4j_connector(sample_temporal_data, monkeypatch):
    ...
    monkeypatch.setattr(
        "agents.analytics.pattern_recognition_agent.NEO4J_AVAILABLE", True
    )
...
def test_neo4j_query_temporal_data(mock_neo4j_connector, sample_temporal_data):
    ...
    data = connector.query_temporal_data(
        "urn:ngsi-ld:Camera:Test", start_time, end_time, ["intensity", "occupancy"]
    )
...
def test_pattern_entity_creation(temp_config, monkeypatch):
    ...
    monkeypatch.setattr(
        "agents.analytics.pattern_recognition_agent.Neo4jConnector.__init__",
        mock_neo4j_init,
    )
    ...
    analysis_results = {
        ...
        "forecast": {"forecast": 0.65, "confidence": 0.75, "method": "moving_average"},
        "anomalies": [],
        "weekly_patterns": {
            "comparison": {
                "weekday_avg": 0.7,
...
def test_analyze_camera_patterns_integration(
    temp_config, mock_neo4j_connector, sample_temporal_data
):
...
def test_rush_hour_accuracy(temp_config):
    ...
    for hour in range(24):
        for day in range(7):
            ...
            data.append({"timestamp": timestamp.isoformat(), "intensity": intensity})
