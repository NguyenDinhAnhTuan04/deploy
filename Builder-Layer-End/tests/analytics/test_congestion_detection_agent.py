...
def make_entity(camera_ref, occ=None, speed=None, intensity=None, observed_at=None):
    if observed_at is None:
        observed_at = now_iso()
    ent = {
        "id": f"urn:ngsi-ld:ItemFlowObserved:{camera_ref}",
    ...
    cfg = {
        "congestion_detection": {
            ...
            "stellio": {
                ...
                "update_endpoint": "/entities/{id}/attrs",
                "batch_updates": True,
            },
            "alert": {"enabled": False},
        }
...
def test_min_duration_starts_timer(tmp_path, monkeypatch):
    ...
    data = (
        ...
        if state_file.exists() and state_file.read_text()
        else None
    )
...
def test_missing_fields_no_crash(tmp_path, monkeypatch):
    ...
    ent = {
        "id": "urn:ngsi-ld:ItemFlowObserved:CamX",
        "refDevice": {"object": "urn:ngsi-ld:Camera:CamX"},
    }
