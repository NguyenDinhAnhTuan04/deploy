...
class TestCVConfig:
    ...
    def test_get_vehicle_classes(self, tmp_path):
        ...
        config_file = tmp_path / "cv_config.yaml"
        config_data = {
            "cv_analysis": {"vehicle_classes": ["car", "motorbike", "bus", "truck"]}
        }
...
class TestYOLOv8Detector:
    ...
    def test_detect_with_mock_model(self):
        ...
        config = {"weights": "yolov8n.pt", "confidence": 0.5, "device": "cpu"}
...
class TestImageDownloader:
    ...
    async def test_download_image_success(self):
        ...
        result = await downloader.download_image(
            mock_session, "http://example.com/image.jpg"
        )
    ...
    async def test_download_image_http_error(self):
        ...
        result = await downloader.download_image(
            mock_session, "http://example.com/notfound.jpg"
        )
    ...
    async def test_download_image_timeout(self):
        ...
        result = await downloader.download_image(
            mock_session, "http://example.com/slow.jpg"
        )
...
class TestNGSILDEntityGenerator:
    ...
    def test_create_item_flow_observed(self):
        ...
        entity = NGSILDEntityGenerator.create_item_flow_observed(
            camera_id=camera_id, location=location, metrics=metrics, timestamp=timestamp
        )
    ...
    def test_create_item_flow_observed_with_detections(self):
        ...
        detections = [
            Detection(
                class_id=2, class_name="car", confidence=0.85, bbox=[10, 20, 100, 200]
            ),
            Detection(
                class_id=2, class_name="car", confidence=0.78, bbox=[200, 30, 300, 250]
            ),
            Detection(
                class_id=3,
...
class TestCVAnalysisAgent:
    ...
    def test_agent_initialization(self, tmp_path):
        ...
        config_data = {
            "cv_analysis": {
                ...
                "batch_size": 20,
                "timeout": 10,
                "metrics": {"intensity_threshold": 0.7, "occupancy_max_vehicles": 50},
    ...
    def test_analyze_image_failure(self, tmp_path):
        ...
        with patch.object(
            agent.detector, "detect", side_effect=Exception("Detection error")
        ):
    ...
    async def test_process_cameras(self, tmp_path):
        ...
        with patch.object(
            agent.downloader,
            "download_batch",
            return_value={"CAM001": test_image, "CAM002": test_image},
        ):
    ...
    async def test_run_with_array_input(self, tmp_path):
        ...
        cameras = [
            {
                "id": "CAM001",
                "imageSnapshot": "http://example.com/1.jpg",
        ...
        with patch.object(
            agent.downloader, "download_batch", return_value={"CAM001": test_image}
        ):
...
class TestIntegration:
    ...
    async def test_end_to_end_workflow(self, tmp_path):
        ...
        test_images = {
            f"CAM{i:03d}": Image.new("RGB", (640, 480), color="blue") for i in range(10)
        }
...
class TestPerformance:
            ...
            cameras.append(
                {
                    ...
                    "location": {
                        "type": "Point",
                        "coordinates": [106 + i * 0.001, 10 + i * 0.001],
                    },
                }
            )
    ...
    async def test_batch_processing_performance(self, tmp_path):
        ...
        cameras = [
            {
                "id": f"CAM{i:03d}",
                "imageSnapshot": f"http://example.com/{i}.jpg",
                "location": {},
            }
            for i in range(50)
        ]
        ...
        with patch.object(
            agent.downloader, "download_batch", side_effect=mock_download_with_delay
        ):
            start_time = time.time()
            entities = await agent.process_cameras(cameras)
            duration = time.time() - start_time
...
class TestEdgeCases:
    ...
    async def test_all_image_downloads_fail(self, tmp_path):
        ...
        cameras = [
            ...
            {
                "id": "CAM002",
                "imageSnapshot": "http://example.com/2.jpg",
                "location": {},
            },
        ]
        ...
        with patch.object(
            agent.downloader,
            "download_batch",
            return_value={"CAM001": None, "CAM002": None},
        ):
            entities = await agent.process_cameras(cameras)
