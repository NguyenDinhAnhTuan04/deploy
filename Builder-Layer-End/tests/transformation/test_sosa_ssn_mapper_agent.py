...
def mock_config_file(tmp_path):
    ...
    config_data = {
        ...
        "platform": {
            ...
            "type": "Platform",
            "properties": {"operator": "Test Operator", "deployment_year": 2020},
        },
        "relationships": {
            "observes": {
                "type": "Relationship",
        ...
        "entity_type_mappings": {
            "Camera": {"add_sensor_type": True},
            "Sensor": {"add_sensor_type": True},
        },
        "output": {
    ...
    entities = [
        {
            ...
            "cameraName": {"type": "Property", "value": "Test Camera 1"},
            "location": {
                "type": "GeoProperty",
        ...
        },
        {
            ...
            "type": "Camera",
            "@context": ["https://uri.etsi.org/ngsi-ld/v1/ngsi-ld-core-context.jsonld"],
            "cameraName": {"type": "Property", "value": "Test Camera 2"},
        },
        {
...
class TestSOSARelationshipBuilder:
    ...
    def test_create_observes_relationship(self):
        ...
        rel = builder.create_observes_relationship(
    ...
    def test_create_observation_relationship_with_timestamp(self):
        ...
        rel = builder.create_observation_relationship(
            "urn:ngsi-ld:Observation:001", observed_at="2025-11-01T12:00:00Z"
        )
...
class TestSOSAValidator:
    ...
    def test_missing_sosa_property(self, mock_config_file):
        ...
        assert any(
            "Missing required SOSA property" in err for err in validator.get_errors()
        )
...
class TestSOSASSNMapperAgent:
    ...
    def test_load_ngsi_ld_entities(
        self, mock_config_file, mock_ngsi_ld_entities, tmp_path
    ):
    ...
    def test_generate_support_entities(self, mock_config_file):
        ...
        obs_prop = next(
            e for e in support_entities if e["type"] == "ObservableProperty"
        )
...
class TestIntegration:
    ...
    def test_full_enhancement_workflow(
        ...
        obs_prop = next(
            e for e in enhanced_entities if e["type"] == "ObservableProperty"
        )
        assert "TrafficFlow" in obs_prop["id"]
