    ...
    return {
        "api_gateway": {
            ...
            "rate_limiting": {
                ...
                "storage": "memory",
                "endpoint_limits": [
                    {"path": "/api/test", "method": "POST", "limit": 50}
                ],
            },
            "routes": [
                {
...
class TestAPIGatewayConfig:
    ...
    def test_env_var_expansion(self, tmp_path):
        ...
        config_data = {"api_gateway": {"server": {"port": "${TEST_PORT:-8000}"}}}
...
class TestTokenBucketRateLimiter:
    ...
    async def test_endpoint_specific_limits(self):
        ...
        config = {
            ...
            "storage": "memory",
            "endpoint_limits": [
                {"path": "/api/restricted", "method": "POST", "limit": 10}
            ],
        }
        limiter = TokenBucketRateLimiter(config)
        ...
        allowed, info = await limiter.check_rate_limit(
            "test-key", None, "GET", "/api/test"
        )
        assert info["limit"] == 100

        # Endpoint-specific limit
        allowed, info = await limiter.check_rate_limit(
            "test-key", None, "POST", "/api/restricted"
        )
...
class TestResponseCache:
    ...
    async def test_cache_miss(self):
        ...
        config = {"enabled": True, "default_ttl": 300, "storage": "memory"}
        cache = ResponseCache(config)

        route_config = {"cache": {"enabled": True, "ttl": 300}}
    ...
    async def test_cache_compression(self):
        ...
        route_config = {"cache": {"enabled": True, "ttl": 300, "compress": True}}
    ...
    async def test_cache_bypass_disabled(self):
        ...
        config = {"enabled": False, "storage": "memory"}
        cache = ResponseCache(config)

        route_config = {"cache": {"enabled": True}}
...
class TestCircuitBreaker:
    ...
    async def test_circuit_closed_allows_requests(self):
        ...
        config = {"enabled": True, "failure_threshold": 5, "recovery_timeout": 60}
        breaker = CircuitBreaker(config)
    ...
    async def test_circuit_opens_on_failures(self):
        ...
        config = {"enabled": True, "failure_threshold": 3, "recovery_timeout": 60}
        breaker = CircuitBreaker(config)
...
class TestAPIGatewayIntegration:
    ...
    def test_authenticated_request_flow(self, gateway_config):
        ...
        with patch.object(
            gateway.http_client, "request", new_callable=AsyncMock
        ) as mock_request:
            mock_response = Mock()
    ...
    def test_rate_limiting_enforcement(self, gateway_config):
        ...
        with patch.object(
            ...
            for i in range(5):
                response = client.get(
                    "/api/test", headers={"X-API-Key": "test-key-123"}
                )
                assert response.status_code == 200
