...
class DataVerifier:
    ...
    def verify_stellio_entities(
        self, entity_type: str, expected_count: int
    ) -> Tuple[bool, int]:
        """
        ...
        try:
            response = self.client.get(url, headers={"Accept": "application/ld+json"})
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"Found {actual_count} entities (expected {expected_count})"
                )
    ...
    def verify_neo4j_count(
        self, node_label: str, expected_count: int
    ) -> Tuple[bool, int]:
        """
    ...
    def verify_fuseki_triples(
        self, dataset: str, expected_min: int
    ) -> Tuple[bool, int]:
        """
        ...
        query_url = urljoin(self.config.fuseki_url, f"/{dataset}/query")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if bindings:
                    ...
                    logger.info(
                        f"Found {actual_count} triples (expected >= {expected_min})"
                    )
...
class TestCompleteSystem:
    ...
    def test_full_pipeline_722_cameras(
        self, test_config, service_readiness, data_verifier, pipeline_executor, cleanup
    ):
        """
        ...
        success, count = data_verifier.verify_fuseki_triples(
            test_config.fuseki_dataset, 8000
        )
    ...
    def test_content_negotiation_e2e(
        self, test_config, service_readiness, pipeline_executor, cleanup
    ):
        """
...
class TestPerformance:
    ...
    def test_api_response_time(self, test_config, service_readiness):
        ...
        url = urljoin(
            test_config.stellio_url, "/ngsi-ld/v1/entities?type=Camera&limit=10"
        )
        ...
        logger.info(
            f"API response times - p50: {p50:.2f}ms, p95: {p95:.2f}ms, p99: {p99:.2f}ms"
        )
    ...
    def test_sparql_query_performance(self, test_config, service_readiness):
        ...
        url = urljoin(test_config.fuseki_url, f"/{test_config.fuseki_dataset}/query")
