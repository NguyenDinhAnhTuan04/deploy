...
class TestCachePolicy:
    ...
    def test_policy_get_max_size_bytes(self):
        ...
        policy1 = CachePolicy(name="test1", pattern="/*", pattern_type=PatternType.EXACT, ttl=60, max_size="5MB")
        policy2 = CachePolicy(name="test2", pattern="/*", pattern_type=PatternType.EXACT, ttl=60, max_size="1024KB")
        policy3 = CachePolicy(name="test3", pattern="/*", pattern_type=PatternType.EXACT, ttl=60, max_size="1GB")
...
class TestCacheManagerIntegration:
    ...
    async def test_cache_set_with_compression(self, config_path, mock_redis):
        ...
        result = await manager.set(cache_key, value, ttl, compress=True, compress_min_size=1024)
...
class TestCacheInvalidator:
    ...
    async def test_invalidate_by_pattern(self, config_path, mock_redis):
        ...
        # Configure mock scan to return keys
        mock_redis.scan.return_value = (0, [b'cache:key1', b'cache:key2', b'cache:key3'])
...
class TestCompressionPerformance:
    ...
    async def test_compression_speed(self, config_path, mock_redis):
        ...
        start = time.time()
        for i in range(100):
            await manager.set(f"cache:test{i}", data, 300, compress=True, compress_min_size=1024)
