...
def sample_workflow_config():
    ...
    return {
        ...
        "retry_policy": {
            ...
            "base_delay": 2,
            "max_delay": 60,
            "retryable_errors": ["ConnectionError", "TimeoutError", "TemporaryFailure"],
        },
        "health_checks": {
            "enabled": True,
            "timeout": 10,
...
def mock_agent_module():
    ...
    mock_module.main = MagicMock(
        return_value={"status": "success", "output": "test_output.json"}
    )
    return mock_module
...
class TestRetryPolicy:
    ...
    def test_should_retry_non_retryable_error(self):
        ...
        policy_config = {"max_attempts": 3, "retryable_errors": ["ConnectionError"]}
        policy = RetryPolicy(policy_config)
    ...
    def test_get_delay_exponential(self):
        ...
        policy_config = {"strategy": "exponential", "base_delay": 2, "max_delay": 60}
        policy = RetryPolicy(policy_config)
    ...
    def test_get_delay_linear(self):
        ...
        policy_config = {"strategy": "linear", "base_delay": 5, "max_delay": 60}
        policy = RetryPolicy(policy_config)
    ...
    def test_get_delay_fixed(self):
        ...
        policy_config = {"strategy": "fixed", "base_delay": 3}
        policy = RetryPolicy(policy_config)
    ...
    def test_retry_with_zero_max_attempts(self):
        ...
        policy_config = {"max_attempts": 0, "retryable_errors": ["ConnectionError"]}
        policy = RetryPolicy(policy_config)
    ...
    def test_retry_with_empty_retryable_errors(self):
        ...
        policy_config = {"max_attempts": 3, "retryable_errors": []}
        policy = RetryPolicy(policy_config)
...
class TestHealthChecker:
    ...
    def test_check_all_with_timeout(self):
        ...
        responses.add_callback(
            responses.GET, "http://localhost:8080/health", callback=timeout_callback
        )
...
class TestPhaseManager:
    ...
    def test_execute_phase_optional_agent_failure(self, mock_agent_module):
        ...
        mock_modules = {
            ...
            "agents.test.agent2": MagicMock(
                main=MagicMock(side_effect=ValueError("Optional failure"))
            ),
        }
...
class TestWorkflowOrchestrator:
    ...
    @responses.activate
    def test_run_workflow_generates_report(
        self, temp_config_file, mock_agent_module, tmp_path
    ):
        ...
        orchestrator = WorkflowOrchestrator(temp_config_file)
        orchestrator.config_loader.config["reporting"]["output_directory"] = str(
            tmp_path
        )
    ...
    def test_run_workflow_parallel_performance(self, tmp_path, mock_agent_module):
        ...
        config_data = {
            ...
            "reporting": {"format": "json", "output_directory": str(tmp_path)},
        }
...
class TestEdgeCases:
    ...
    def test_workflow_with_no_phases(self, tmp_path):
        ...
        config_data = {
            "workflow": {"name": "Empty Workflow", "version": "1.0.0", "phases": []},
    ...
    @responses.activate
    def test_workflow_continues_after_optional_failure(
        self, tmp_path, mock_agent_module
    ):
        ...
        # Mock modules
        mock_modules = {
            "agents.test.optional": MagicMock(
                main=MagicMock(side_effect=ValueError("Optional failure"))
            ),
