    ...
    return {
        'performance_monitor': {
            ...
            'alerting': {
                ...
                'notifications': {
                    'channels': [
                        {'type': 'log', 'level': 'warning', 'enabled': True}
                    ]
                },
                'rules': [
                    {
...
class TestPerformanceMonitorConfig:
    ...
    def test_env_var_expansion(self, config_file):
        ...
        try:
            ...
            assert config.config['performance_monitor']['prometheus']['host'] == 'test_value'
        finally:
            os.unlink(temp_path)
...
class TestNeo4jMetricsCollector:
    ...
    def test_close_connection(self, mock_driver, config):
        ...
        mock_driver_instance.session.return_value.__enter__.return_value.run.return_value = None
...
class TestTrendAnalysisEngine:
    ...
    def test_get_trend_statistics(self, config):
        ...
        for i, val in enumerate(values):
            engine.record_metric('cpu_percent', val, current_time - (len(values) - i) * 60)
    ...
    def test_detect_anomalies_zscore(self, config):
        ...
        current_time = time.time()
        for i in range(100):
            engine.record_metric('cpu_percent', 50.0 + (i % 10), current_time - (100 - i) * 60)
...
class TestPerformanceMonitorAgentIntegration:
    ...
    def test_collect_and_export(self, mock_memory, mock_cpu, mock_driver, config_file):
        ...
        # Mock Neo4j
        mock_driver_instance = MagicMock()
        mock_driver_instance.session.return_value.__enter__.return_value.run.return_value.single.return_value = None
        mock_driver_instance.session.return_value.__exit__.return_value = None
        mock_driver.return_value = mock_driver_instance
...
class TestPerformanceUnderLoad:
    ...
    def test_concurrent_metric_collection(self, config):
        ...
        def collect_metrics(agent_id):
            try:
                for i in range(1000):
                    collector.record_agent_execution(f'agent_{agent_id}', 1.0, 'success')
            except Exception as e:
                errors.append(e)
        
        # Start 10 threads
        threads = [threading.Thread(target=collect_metrics, args=(i,)) for i in range(10)]
        start_time = time.time()
    ...
    def test_alert_evaluation_performance(self, config):
        ...
        current_time = time.time()
        for i in range(1000):
            manager.record_metric_value('cpu_percent', 50.0 + (i % 50), current_time - (1000 - i))
    ...
    def test_trend_analysis_performance(self, config):
        ...
        current_time = time.time()
        for i in range(10000):
            engine.record_metric('cpu_percent', 50.0 + (i % 50), current_time - (10000 - i) * 60)
