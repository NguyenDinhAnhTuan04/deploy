"""
External Data Collector Agent - 100% Domain-Agnostic & Config-Driven

Enriches entities (cameras, sensors, devices, etc.) with external contextual data
(weather, air quality) by reading YAML config and adding fields to ORIGINAL objects.

MANDATORY Requirements:
✅ 100% domain-agnostic: Works with ANY entity type (cameras, medical devices, inventory)
✅ 100% config-driven: ALL endpoints, keys, rate limits in config/data_sources.yaml
✅ Async/await with aiohttp for parallel API calls
✅ Retry logic with exponential backoff (3 attempts, configurable delays)
✅ Caching with async-lru (10-minute TTL)
✅ Rate limiting (token bucket per API)
✅ Enriches ORIGINAL camera objects (adds weather{} and air_quality{} fields)
✅ Output: cameras_enriched.json with ALL original fields + external data

Architecture:
- Input: data/cameras_updated.json (40 cameras with lat/lng)
- APIs: OpenWeatherMap (weather), OpenAQ (air quality)
- Config: config/data_sources.yaml (ALL settings)
- Output: data/cameras_enriched.json (ORIGINAL cameras + weather + air_quality)
- Features: async parallel calls, retry with backoff, caching, rate limiting

Author: Builder Layer LOD System
Version: 2.0.0 (MANDATORY Requirements Compliant)
"""
...
class ExternalDataCollectorAgent:
    ...
    100% domain-agnostic: Works with ANY LOD domain (healthcare, geography, commerce)
    100% config-driven: All configurations in YAML file
