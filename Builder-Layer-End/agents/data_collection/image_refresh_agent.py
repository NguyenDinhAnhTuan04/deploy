"""
Image Refresh Agent - Domain-Agnostic Data Collection

This agent refreshes time-sensitive URLs (e.g., camera images, sensor data) by:
1. Loading endpoints from YAML configuration
2. Parsing URLs to extract parameters
3. Generating fresh timestamps
4. Rebuilding URLs with updated timestamps
5. Verifying URL accessibility via HTTP HEAD requests
6. Outputting updated data in JSON format

ARCHITECTURE:
- 100% domain-agnostic: Works with ANY LOD domain via config
- 100% config-driven: All endpoints, mappings in YAML
