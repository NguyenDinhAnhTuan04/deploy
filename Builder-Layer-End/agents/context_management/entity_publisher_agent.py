"""
Entity Publisher Agent - NGSI-LD Entity Publication to Stellio Context Broker

This agent provides 100% domain-agnostic publishing of NGSI-LD entities to Stellio Context Broker.
It works with ANY LOD domain (healthcare, geography, commerce, IoT, etc.) without code changes.
...
- Performance tracking and detailed reports

Architecture:
- ConfigLoader: Load and validate Stellio configuration from YAML
- BatchPublisher: Handle HTTP requests to Stellio with retry logic
- PublishReportGenerator: Track and report publishing statistics
- EntityPublisherAgent: Main orchestrator for the publishing workflow
...
class ConfigLoader:
    ...
    def _apply_env_overrides(self) -> None:
        ...
        if "STELLIO_BASE_URL" in os.environ:
            ...
            logger.info(
                f"Base URL overridden from environment: {stellio_config['base_url']}"
            )
...
class EntityPublisherAgent:
    ...
    This agent provides:
    - 100% domain-agnostic entity publishing
