    ...
    def _patch_entity(self, entity_id: str, payload: Dict[str, Any]) -> Tuple[bool, Optional[int], Optional[str]]:
        ...
        try:
            ...
            if self.stellio_base:
                url = self.stellio_base.rstrip('/') + self.update_endpoint.format(id=entity_id)
            else:
        ...
        if to_update:
            if self.batch_updates:
                ...
                with ThreadPoolExecutor(max_workers=self.max_workers) as exe:
                    futures = {exe.submit(self._patch_entity, cam, payload): (cam, payload, ent, new_st) for cam, payload, ent, new_st in to_update}
                    for fut in as_completed(futures):
                        ...
                        try:
                            ...
                            status_code = None
                            error = str(e)
                        update_results.append({'camera': cam, 'updated': True, 'success': success, 'status_code': status_code, 'error': error})
                        if success:
                            ...
                            if new_st:
                                ...
                                self.state_store.update(cam, True, fb, payload.get('congested', {}).get('observedAt'))
                            else:
                                # Clear congestion
                                self.state_store.update(cam, False, None, payload.get('congested', {}).get('observedAt'))
                            # Alert if notify_on_change and previous was False
                            if self.alert_cfg.get('enabled', False) and self.alert_cfg.get('notify_on_change', False):
                                if not prev.get('congested', False) and new_st:
                                    self._alert(cam, ent, payload.get('congested', {}).get('observedAt'))
                        else:
