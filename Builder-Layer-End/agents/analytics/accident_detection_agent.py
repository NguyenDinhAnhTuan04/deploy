...
Domain-agnostic anomaly detection framework for identifying traffic accidents
from ItemFlowObserved entities and creating RoadAccident NGSI-LD entities.

Features:
- Multiple detection methods (statistical, rule-based, pattern-based)
- Severity classification (minor, moderate, severe)
- False positive filtering with cooldown
- Batch entity creation in Stellio
- Alert generation for significant events
- State persistence and history tracking

Architecture:
- AccidentConfig: Load and validate YAML configuration
- StateStore: Persist detection state and history
- DetectionMethod: Base class for anomaly detection algorithms
- SpeedVarianceDetector: Statistical detection via speed variance
- OccupancySpikeDetector: Rule-based detection via occupancy spike
- SuddenStopDetector: Rule-based detection via sudden speed drop
- PatternAnomalyDetector: Statistical detection via intensity patterns
- AccidentDetectionAgent: Main orchestrator

Usage:
    python agents/analytics/accident_detection_agent.py data/observations.json
    python agents/analytics/accident_detection_agent.py data/observations.json --config path/to/config.yaml
"""
