...
This agent performs computer vision analysis on camera images using YOLOv8 object detection.
It detects vehicles (cars, motorbikes, buses, trucks) and calculates traffic metrics like
vehicle count, intensity, occupancy, and estimated speed.

The agent is 100% domain-agnostic and config-driven - it works with any image source
and can be adapted to any domain (traffic, parking, warehouse, retail) via YAML configuration.

Features:
- Async batch image download
    ...
    async def download_image(
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.warning(
                    f"{log_prefix} Connection error: {type(e).__name__}: {e}"
                )
