...
export class StellioService {
   ...
   * Queries: GET /entities?type=Camera&limit=100
  ...
  private async transformWeatherEntity(entity: any): Promise<Weather> {
    ...
    // Try to fetch camera location if refDevice exists
    if (refDevice) {
      const cameraIdFromRef = typeof refDevice === 'string' ? refDevice : refDevice.id || refDevice['@id'];
      if (cameraIdFromRef) {
        ...
        try {
          ...
          logger.warn(`Failed to fetch camera ${cameraId} for weather entity ${entity.id}:`, error);
        }
      }
    }

    // Fallback to entity's own location if camera lookup failed
    if (cameraId === 'unknown' && entity.location) {
  ...
  private async transformAirQualityEntity(entity: any): Promise<AirQuality> {
    ...
    if (refDevice) {
      ...
      if (cameraIdFromRef) {
        ...
        try {
          ...
          logger.warn(`Failed to fetch camera ${cameraId} for air quality entity ${entity.id}:`, error);
        }
      }
    }
    ...
    const aqi = this.extractNumericValue(entity.aqi || entity.airQualityIndex, 50) || 50;
   ...
   * Queries: GET /entities?type=TrafficFlowPattern&limit=100
