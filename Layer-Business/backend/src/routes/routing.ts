...
function calculateAQIScore(route: OSRMRoute, zones: VoronoiZone[]): number {
  ...
  return Math.min(100, (weightedAQI / 500) * 100);
}
...
function calculateWeatherScore(route: OSRMRoute, zones: VoronoiZone[]): number {
  ...
  return (rainfallScore * 0.6 + visibilityScore * 0.4);
}
...
function calculateTrafficScore(route: OSRMRoute, zones: VoronoiZone[]): number {
  ...
  return congestionScores.reduce((sum, score) => sum + score, 0) / congestionScores.length;
}
...
router.post('/calculate', async (req: Request, res: Response) => {
  try {
    ...
    const calculatedRoutes: CalculatedRoute[] = osrmRoutes.map(route => {
