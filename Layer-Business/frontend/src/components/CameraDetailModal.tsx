...
const CameraDetailModal: React.FC<CameraDetailModalProps> = ({ camera, onClose, onViewOnMap }) => {
  ...
  const cameraLat = camera.location.latitude || (camera.location as any).lat || 0;
  const cameraLng = camera.location.longitude || (camera.location as any).lng || 0;
  ...
  const currentWeather = useMemo(() => {
    ...
    return weather.find(w => {
      const distance = calculateDistance(
        cameraLat,
        cameraLng,
        w.location.latitude || w.location.lat || 0,
        w.location.longitude || w.location.lng || 0
      );
    ...
    return airQuality.find(a => {
      const distance = calculateDistance(
        cameraLat,
        cameraLng,
        a.location.latitude || a.location.lat || 0,
        a.location.longitude || a.location.lng || 0
      );
