...
const TimeMachine: React.FC<TimeMachineProps> = ({ visible, onClose, onDataUpdate }) => {
  ...
  return (
    <div className="fixed bottom-0 left-1/2 transform -translate-x-1/2 z-[9999] w-full max-w-5xl">
      <div className="bg-gradient-to-br from-gray-900 via-purple-900 to-gray-900 rounded-t-xl shadow-2xl border-t border-l border-r border-purple-500/30 backdrop-blur-xl max-h-[45vh] overflow-y-auto">
        {/* Header */}
        <div className="bg-gradient-to-r from-purple-600 via-fuchsia-600 to-purple-600 text-white px-4 py-2 flex items-center justify-between border-b border-white/20 sticky top-0 z-10">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={handleClose}
              className="bg-gradient-to-br from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 rounded-lg p-2 transition-all hover:scale-110 active:scale-95 shadow-2xl border-2 border-yellow-300"
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="flex items-center gap-3">
            <label className="text-xs font-bold text-white/90 whitespace-nowrap">Date Range:</label>
            <div className="flex gap-2">
              {(['7days', '14days', '30days'] as DateRange[]).map(range => (
                <button
                  key={range}
                  onClick={() => setDateRange(range)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all shadow-lg ${dateRange === range
          ...
          {/* Date Picker */}
          <div className="flex items-center gap-3">
            <label className="text-xs font-bold text-white/90 whitespace-nowrap">Select Date:</label>
            <input
              type="date"
              ...
              className="flex-1 px-3 py-1.5 bg-white/10 border border-white/20 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent text-white text-sm font-medium backdrop-blur-sm"
            />
            <span className="text-xs text-white/80 whitespace-nowrap bg-white/10 px-3 py-1.5 rounded-lg font-medium border border-white/20">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <label className="text-xs font-bold text-white/90">Time:</label>
            ...
            <div className="relative">
              <input
                ...
                className="w-full h-2 bg-white/20 rounded-full appearance-none cursor-pointer slider-purple border border-white/20"
              />
              {/* Hour markers */}
              <div className="flex justify-between mt-2 text-[10px] text-white/60 font-medium">
                {Array.from({ length: 24 }, (_, i) => i).filter(h => h % 3 === 0).map(hour => (
                  <span key={hour} className="bg-white/10 px-1.5 py-0.5 rounded">{hour.toString().padStart(2, '0')}</span>
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between pt-3 border-t border-white/20">
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={togglePlay}
                className="bg-gradient-to-r from-purple-500 to-fuchsia-500 hover:from-purple-600 hover:to-fuchsia-600 text-white p-2.5 rounded-full transition-all shadow-xl hover:scale-110"
              ...
              <button
                onClick={cycleSpeed}
                className="bg-white/10 hover:bg-white/20 text-white px-3 py-1.5 rounded-lg text-sm font-bold transition-all border border-white/20 shadow-lg"
              ...
              {/* Loading Indicator */}
              {isLoading && (
                <div className="flex items-center gap-1.5 text-xs text-white bg-white/10 px-3 py-1.5 rounded-lg border border-white/20">
            ...
            <div className="flex items-center gap-3 text-xs">
              <div className="text-white/70 bg-white/10 px-3 py-1.5 rounded-lg border border-white/20">
                <span className="font-bold text-white">Camera Locations:</span> {cameras.length}
              </div>
              <div className="text-purple-300 font-bold bg-purple-500/20 px-3 py-1.5 rounded-lg border border-purple-400/30">
